...
open class UnderLineTextField: UITextField {
    ...
    ///Bottom line
    public var isBottomLine: Bool = false {
        didSet {
            setNeedsDisplay()
        }
    }
    ...
    public var isPlaceHolderAnimation: Bool = true {
        didSet {
            setNeedsDisplay()
        }
    }
    ...
    @IBInspectable open var inactiveLineWidth: CGFloat = 0.5 {
        didSet {
    ...
    @IBInspectable open var warningLineWidth: CGFloat = 0.5 {
        didSet {
    ...
    @IBInspectable open var errorLineWidth: CGFloat = 0.5 {
        didSet {
...
extension UnderLineTextField {
    ...
    private func setPlaceholderPlace(isUp: Bool, isAnimated: Bool) {
        ...
        if !isPlaceHolderAnimation {
            placeholderLabel.isHidden = isUp
            return
        }
    ...
    private func createLinePath() -> UIBezierPath {
        ...
        path.move(to: CGPoint(x: 0, y: isBottomLine ? bounds.maxY : padding))
        path.addLine(to: CGPoint(x: bounds.maxX, y: isBottomLine ? bounds.maxY : padding ))
        return path
    }
